...
export async function GET(request: Request) {
  try {
    ...
    // 获取完整的用户信息（包括个人资料）
    const usersWithProfiles = readers.map(reader => {
      const profile: any = getUserProfile(reader.id);
      // 获取用户认证信息以确定角色
      const authInfo: any = getUserByUsername(reader.userId);
      return {
        ...reader,
        ...profile,
    ...
    return NextResponse.json({ 
      ...
      users: usersWithProfiles.map(user => ({
        ...
        full_name: user.full_name || user.userId || `用户${user.id}`,
        username: user.userId || `user${user.id}`,
        email: user.email || `${user.userId}@ibookstore.com`,
        creditLevel: user.creditLevel || 1,
        joined: '2023-01-01', // 这里应该从用户认证表获取注册时间
